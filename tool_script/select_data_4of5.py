...
def copy_lines_without_interval(source_file, destination_file, start_index=2, interval=5):
    # 确保目标文件夹存在
...
def copy_files_with_interval_and_rename(source_folder, destination_folder, start_index=2, interval=5):
    # 确保目标文件夹存在
    if not os.path.exists(destination_folder):
        os.makedirs(destination_folder)
...
if 1:
    ...
    destination_folder_path = "/home/biter/paper2/kitti/10_4of5/velodyne"
    source_file_path = "/home/biter/paper2/kitti/10_base/poses_lidar.txt"
    destination_file_path = "/home/biter/paper2/kitti/10_4of5/poses_lidar.txt"
...
copy_files_with_interval_and_rename(source_folder_path, destination_folder_path, start_index=2, interval=5)

# 例子：从a.txt文件中，从第三行开始，每隔5行不保留一个，将剩余的行复制到b.txt文件中
copy_lines_without_interval(source_file_path, destination_file_path, start_index=2, interval=5)